 ...
 * 定义数组
 */
FTM_MemMapPtr FTMN[FTM_MAX] = {FTM0_BASE_PTR, FTM1_BASE_PTR, FTM2_BASE_PTR, FTM3_BASE_PTR}; //定义三个指针数组保存 FTMn_e 的地址
...
void ftm_pwm_init(FTMn_e ftmn, FTM_CHn_e ch, uint32 freq, uint32 duty)
{
    ...
    // 以 CPWMS = 0 ，即单边捕捉脉冲为例
    clk_hz = (bus_clk_M * 1000*1000) ;     // bus频率
    ...
    FTM_SC_REG(FTMN[ftmn])    = ( 0
                                  ...
                                  | FTM_SC_CLKS(1)            //时钟选择， 0：没选择时钟，禁用； 1：bus 时钟； 2：MCGFFCLK； 3：EXTCLK（ 由 SIM_SOPT4 选择输入管脚 FTM_CLKINx）
...
{
    uint32 clk_hz = (bus_clk_M * 1000 * 1000) >> 1;        //bus频率/2
    uint32 mod;
    uint8 ps;
